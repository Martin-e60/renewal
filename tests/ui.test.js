...
test('pricing explains both plans and contact has a usable email fallback plus FAQ',async()=>{
 ...
 assert.match(h.document.querySelector('.pro-plan [data-checkout]').textContent,/Upgrade now/);
 assert.doesNotMatch(h.document.querySelector('.pro-plan [data-checkout]').textContent,/12\.00|month/);
 assert.match(h.document.querySelector('.pro-plan .plan-price').textContent,/€12\.00\s*per month/);
...
test('transactions tab keeps currencies apart, excludes income and transfers, and filters',async()=>{
 ...
 const localSearch=h.document.querySelector('[data-tx-search]');assert.ok(localSearch);localSearch.value='Netflix';localSearch.dispatchEvent(new h.Event('input'));await h.tick();assert.equal(h.document.querySelectorAll('[data-tx]').length,1);assert.equal(h.document.querySelector('[data-global-search]').value,'Netflix');
 h.document.querySelector('[data-tx-search]').value='';h.document.querySelector('[data-tx-search]').dispatchEvent(new h.Event('input'));await h.tick();
...
test('import wizard reads the CSV, explains invalid rows and sends reviewed choices',async()=>{
 ...
 assert.equal(dialog.querySelectorAll('select').length,0,'the import wizard never falls back to browser-owned select menus');
 const kind=dialog.querySelector('[data-row-kind="1"]');pick(kind.closest('[data-tx-picker]'),'transfer');
