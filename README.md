...
## Transactions — statement import and spending

- The Subscriptions tab is now **Transactions** (`/dashboard/transactions`). Old `/dashboard/subscriptions` links, including reminder emails, open its Subscriptions view. Views: All transactions, Subscriptions (tracked subscriptions, as before) and To review (possible subscriptions).
- **Import statement** takes a CSV bank export by file picker or drag and drop, for a chosen or new account. Supported: UTF-8, UTF-16 and Windows-1251; comma, semicolon, tab or pipe delimiters; preamble lines above the header; signed amounts, separate money-in/money-out columns or a debit/credit marker; decimal comma or point. When headers are not recognised, the user matches the columns. Invalid rows are listed with the reason and are never imported.
- The file is read in the browser and nothing is saved until **Confirm import**. The server re-validates every row, categorises it (Food & groceries, Transport, Shopping, Entertainment, Bills & utilities, Health, Other) and separates income, refunds and transfers between the user's own accounts (keywords, or the opposite amount in another of the user's accounts within three days). Spending is expenses minus refunds; income and own transfers are shown separately and not counted. Type and category can be changed before and after the import.
- Duplicates: every row gets a fingerprint (date, amount, currency, description and its occurrence within the file), unique per account, so uploading the same or an overlapping statement again skips rows that are already saved. Rows with a saved amount within one day but a different description are flagged as possible duplicates and excluded by default.
- Possible subscriptions are two or more payments to the same merchant in the same currency at a weekly (three or more), monthly or yearly interval with similar amounts. Confirming a EUR subscription adds it to the renewal tracker (radar, calendar, reminders), or links it to an existing record with the same name. Confirmed non-EUR subscriptions are listed in their own currency. Rejections are remembered and can be moved back to review.
- Totals and category breakdowns are per currency for the selected period, account, category and type. Currencies are never converted or added together. The global search also filters transactions.
- **Add transaction** records a manual expense, income, refund or own-account transfer for an existing or new account (for example, a cash wallet). Enter a positive amount and choose the direction for transfers. Manual entries have a badge and do not count as bank imports; retrying the same submission does not create duplicates.
- Check expenses and refunds on the current page to **Change category** together. **Remember this category for these merchants** is optional and off by default: it saves a rule for future transactions with the same normalised merchant, only for this user. Unselected history stays unchanged and explicit import-review corrections take priority. **Saved category rules** lets the user remove a rule without changing saved transactions.
- The freshness card shows the last import time, latest imported transaction date and latest transaction date including manual entries. It follows the selected account independently of other filters; bank data is refreshed by another statement import.
- Transactions can be sorted by date or signed amount and paged in groups of 25 or 50. Amount sorting groups currencies separately. Totals always use all filtered transactions, not just the visible page. Selection is limited to the current page and cleared when filters, sorting or pages change.
- Removing an import deletes its transactions and keeps tracked subscriptions. Transactions are stored in their own tables and deleted with the account.
- When an imported row pairs with the opposite amount in another of the user's accounts, the existing counterpart is also marked as a transfer, so neither side counts as spending or income.
- Not included yet: transactions in the JSON backup export/import, renaming or deleting bank accounts, and PDF/OFX/MT940 statements.
- Layout: the page uses a temporary functional layout in `public/assets/transactions.css`, built on the tokens and components in `styles.css`. Behaviour is bound to `data-*` attributes (`data-tx-*`, `data-row-*`, `data-series*`, `data-import-*`), so the stylesheet and `tx-*` classes can be replaced by the final design without touching logic. Tables become stacked cards below 620 px.
- `tests/transactions-e2e.test.js` runs the real `app.js` UI code against a real server and a temporary database through the whole workflow. With `TX_SNAPSHOTS=<dir>` it also saves each screen as static HTML for visual review.
