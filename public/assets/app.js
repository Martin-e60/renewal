...
// ---------- Transactions ----------
function emptyTransactions(){return {loaded:false,error:'',accounts:[],transactions:[],series:[],imports:[]};}
async function loadTransactions(){const accountId=state.user?.id;try{const data=await api('/api/transactions');if(state.user?.id!==accountId)return;state.tx={loaded:true,error:'',accounts:data.accounts||[],transactions:data.transactions||[],series:data.series||[],imports:data.imports||[]};}catch(e){if(state.user?.id===accountId)state.tx={...emptyTransactions(),error:e.message};}}
...
function allTransactionsView(){
 if(!state.tx.transactions.length)return `<div class="empty-state card tx-empty"><h3>No transactions yet</h3><p>Import a CSV bank statement to see your spending by category.</p><button class="btn btn-primary" data-import-statement>${svgIcon('upload',18)} Import statement</button></div>${importsList()}`;
...
function transactionResults(){
 ...
 const rows=list.slice(0,500).map(t=>{const status=statuses.get(`${t.merchantKey}|${t.currency}`);return `<tr data-tx="${esc(t.id)}"><td data-label="${cellLabel('Date')}">${shortDate(t.date)}</td><td class="tx-desc"><span data-user-content>${esc(t.description)}</span>${status==='confirmed'?' <span class="badge">Subscription</span>':status==='possible'?' <span class="badge">Possible subscription</span>':''}</td><td data-label="${cellLabel('Account')}"><span data-user-content>${esc(bankAccountName(t.accountId))}</span></td><td data-label="${cellLabel('Type')}"><select class="tiny-select" aria-label="Type" data-tx-kind="${esc(t.id)}">${kindOptions(t.kind)}</select></td><td data-label="${cellLabel('Category')}">${isSpendingKind(t.kind)?`<select class="tiny-select" aria-label="Category" data-tx-category="${esc(t.id)}">${categoryOptions(t.category)}</select>`:'—'}</td><td data-label="${cellLabel('Amount')}" class="tx-num ${t.amount>0?'tx-in':''}">${moneyIn(t.amount,t.currency)}</td></tr>`;}).join('');
 return `${summary?`<section class="tx-summary">${summary}</section>`:''}<p class="tx-result-count"><strong data-tx-count>${list.length}</strong> <span>transactions</span></p><div class="card tx-table-card">${list.length?`<div class="tx-table-scroll"><table class="tx-table"><thead><tr><th>Date</th><th>Description</th><th>Account</th><th>Type</th><th>Category</th><th class="tx-num">Amount</th></tr></thead><tbody>${rows}</tbody></table></div>`:'<p class="empty-state">No transactions match these filters.</p>'}${list.length>500?'<p class="tx-table-foot">Showing the latest 500 transactions. Narrow the filters to see the rest.</p>':''}</div>`;
}
...
function bindTransactionRows(root){
 root.querySelectorAll('[data-tx-kind],[data-tx-category]').forEach(select=>select.addEventListener('change',async()=>{
  ...
  try{const updated=await api('/api/transactions/'+id,{method:'PATCH',body:JSON.stringify(change)});Object.assign(item,{kind:updated.kind,category:updated.category});updateTransactionResults();}
...
function bindTransactions(){
 ...
 document.querySelectorAll('[data-tx-view]').forEach(b=>b.addEventListener('click',()=>{state.txView=b.dataset.txView;render();}));
...
function clearIdentity(){state.user=null;state.tx=emptyTransactions();state.subscriptions=[];state.documents=[];state.pro=false;state.billing={};state.version=0;state.dataError='';state.remotePending=false;closeModal();}
...
function bindInteractions(){bindBilling(app);
  ...
  document.querySelector('[data-logout]')?.addEventListener('click',async()=>{try{await api('/api/auth/logout',{method:'POST',body:'{}'});state.user=null;state.subscriptions=[];state.documents=[];state.pro=false;state.tx=emptyTransactions();toast('You’re signed out.','success');navigate('/');}catch(e){toast(e.message,'error');}});
  document.querySelectorAll('[data-auth-form]').forEach(bindAuthForm);
  document.querySelector('[data-global-search]')?.addEventListener('input',e=>{state.dashboardQuery=e.target.value;if(location.pathname!=='/dashboard/transactions'&&state.dashboardQuery.trim()){state.subscriptionFilter='All';const focus=()=>document.querySelector('[data-global-search]')?.focus();Promise.resolve(navigate('/dashboard/transactions')).then(focus);focus();}else{applySubscriptionFilters();updateTransactionResults();}});
