...
export const dayDifference = (a, b) =>
  Math.round((stamp(a) - stamp(b)) / 86400000);
export function addDays(s, n) {
  return new Date(stamp(s) + n * 86400000).toISOString().slice(0, 10);
}
...
export function nextRenewal(s, from = dateKey()) {
  ...
  if (!validDate(anchor) || !validDate(from))
    throw new Error("Invalid renewal date");
  if (from <= anchor) return anchor;
  const [ay, am, ad] = anchor.split("-").map(Number),
    [fy, fm] = from.split("-").map(Number);
  if (s.cycle === "Weekly")
    return addDays(anchor, Math.ceil(dayDifference(from, anchor) / 7) * 7);
  if (s.cycle === "Yearly") {
    let result = monthDate(fy, am - 1, ad);
    return result >= from ? result : monthDate(fy + 1, am - 1, ad);
  }
  let result = monthDate(fy, fm - 1, ad);
  return result >= from ? result : monthDate(fy, fm, ad);
}
export function monthRenewals(s, year, month) {
  const start = monthDate(year, month, 1),
    end = monthDate(year, month + 1, 1),
    out = [];
...
export const annualAmount = (s) =>
  (Math.round(Number(s.price) * 100) *
    (s.cycle === "Yearly" ? 1 : s.cycle === "Weekly" ? 52 : 12)) /
  100;
